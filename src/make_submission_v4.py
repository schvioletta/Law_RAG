"""Strong pure retrieval: answer-expansion + BM25/TFIDF + e5/BGE/e5-ft + LTR."""
...
def load_dense(path, docs, chunk_size, overlap, max_chunks, prefix_passages=False):
    data = np.load(path)
...
def main():
    ...
    train_ans = [tok(str(t)) for t in train.ideal_answer]
    ...
    def expand(q: str, n=8, add=20) -> list[str]:
        base = tok(q)
        ...
        for j in np.argsort(-sims):
            ...
            if str(train.iloc[j].question) == q:
                continue
            for t in train_ans[j] + train_ev[j]:
                if len(t) >= 4:
    ...
    def bm25_scores(q_tokens):
        ...
        ranked = rrf(
            [
                [doc_ids[i] for i in np.argsort(sd)[::-1][:180]],
                [d for d, _ in sorted(best.items(), key=lambda x: -x[1])[:180]],
            ],
            weights=[1.0, 1.2],
        )
    ...
    def dense_scores(qe, chunk_emb, full_emb, dens_docs):
        ...
        ranked = [d for d, _ in sorted(best.items(), key=lambda x: -x[1])[:180]]
    ...
    def first_stage(q: str):
        ...
        tf = rrf(
            [
                [doc_ids[i] for i in np.argsort(sw)[::-1][:180]],
                [doc_ids[i] for i in np.argsort(sc)[::-1][:180]],
            ]
        )
        fused = rrf(
            [bm1, bm2, e5r, bger, ftr, tf],
            weights=[1.35, 1.25, 0.95, 1.05, 1.35, 0.8],
        )[:CAND]
        return {
            "cands": fused,
            "sd": sd,
            ...
            "sc": sc,
            "bm1": bm1,
            "dense": rrf([e5r, bger, ftr], weights=[0.9, 1.0, 1.3]),
        }
    ...
    def make_feats(q, pack):
        ...
        dens_rank = {d: r for r, d in enumerate(pack["dense"])}
        X = []
        for rank, d in enumerate(cands):
            ...
            X.append(
                [
                    ...
                    float(uniq),
                    float(lens[i]),
                    float(bm_rank.get(d, 300)),
                    float(dens_rank.get(d, 300)),
                ]
            )
    ...
    for qi, row in tqdm(train.iterrows(), total=len(train)):
        ...
        # for training only: ensure positive is present
        if g not in cands:
            cands = cands[:-1] + [g]
        pack = dict(pack)
        pack["cands"] = cands
    ...
    print("CV (with gold-in-cand for learning; honest eval without injection)...")
    gkf = GroupKFold(5)
    recalls_trainlike, recalls_honest, iters = [], [], []
    for fold, (tr, te) in enumerate(gkf.split(X, y, groups)):
        ...
        # honest: re-rank only original first-stage cands (no gold injection)
        hits_h = nq_h = 0
        te_qids = sorted(set(int(g) for g in groups[te]))
        for qi in te_qids:
            row = train.iloc[qi]
            q = str(row.question)
            ...
            order = [pack["cands"][i] for i in np.argsort(-scores)]
            # blend with first-stage
            final = rrf([order, pack["cands"]], weights=[1.7, 0.9])[:TOP_K]
            if gold in final:
                hits_h += 1
            nq_h += 1
        honest = hits_h / nq_h
        ...
        recalls_trainlike.append(trainlike)
        recalls_honest.append(honest)
        iters.append(booster.best_iteration or 150)

    cv_h = float(np.mean(recalls_honest))
    cv_t = float(np.mean(recalls_trainlike))
    n_est = int(np.median(iters))
    print(f"CV honest Recall@5={cv_h:.4f} trainlike={cv_t:.4f} rounds={n_est}")
    ...
    def retrieve(q: str):
        ...
        return rrf([ltr, pack["cands"], pack["bm1"][:CAND]], weights=[1.7, 0.9, 0.6])[
            :TOP_K
        ]
    ...
    with open(OUT / "metrics_v4.txt", "w") as f:
