...
**Сейчас в репозитории — v15b equal fuse** (+4 epoch continue FT) (не откат к 0.52):

`RRF([v6, v15_ltr], weights=[1.0, 1.0])`

- отличается от v6 на **239/350** вопросов по составу top-5
- `train_gold_frac ≈ 0.597` (как у v6 ≈0.589)
- локально: nested RRF ≈ **0.55** (честный GroupKFold), LTR CV ≈ **0.657**
...
| **v15** | nested e5-small FT + BM25 + LTR ⊕ v6 equal | nested RRF ≈**0.55**; LTR CV ≈**0.62**; set_diff≈**231** |
| **v15b** | v15 +4 epoch continue FT ⊕ equal fuse | LTR CV ≈**0.657**; set_diff≈**239**; tg≈**0.597** ← текущий submit |
...
4. Continue FT (+4 epoch) на `e5_nested_v15` → `e5_nested_v15b`.
5. **Equal fuse с pinned v6**: `RRF([v6, v15b_ltr], weights=[1.0, 1.0])` — set_diff≈239, `train_gold_frac≈0.597`.
